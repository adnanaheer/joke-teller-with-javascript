...
function tellMe(joke, dev) {

    VoiceRSS.speech({
        key: 'a8ce1f2aa64e45c185d90cf5159c457c',
        src: joke,
        // src: dev,
        hl: 'en-us',
        v: 'Linda',
...
async function getJokes() {
    let joke = ''
    let dev = 'I Am A Robot Adnan A...R'
    const apiUrl = 'https://v2.jokeapi.dev/joke/Programming,Miscellaneous,Dark,Pun,Spooky?blacklistFlags=nsfw,religious,political,racist,explicit';
    try {
        const response = await fetch(apiUrl)
        const data = await response.json()
        if (data.setup, dev) {
            joke = `${data.setup} ... ${data.delivery}`
            dev = 'I am creating a Robot for joking ...'
        } else {
            joke = data.joke
            dev = 'My Robot is not correct woking now ...'
        }
        tellMe(joke, dev)
        toggleButton()
    } catch (error) {
